...
public class LastLevel extends MenuOptions{
	...
	public LastLevel(Game g, Inputs k) {
		game = g;
		key = k;
		...
		things.add(new Planet(350,200, Identifier.Still));
		things.add(new Planet(200,400, Identifier.Still));
		...
		p = new Portal(600,600, Identifier.Portal);
	...
	public void updateState() {
		...
		if (c.items() == 8) {
			things.add(p);
			if (spawn % 300 == 0) {
				things.add(new Boss(600,600, Identifier.Boss, this, things));
			}
		}
		
		if (c.hasWon()) {
			MenuOptions.setOption(game.winState);
			music.stopMusic();
			music.setSong("#3.wav");
			music.startMusic();
		}

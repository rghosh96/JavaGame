...
public class DoGame extends MenuOptions{
	...
	public DoGame(Game g, Inputs k) {
		...
		things.add(new Planet(400,400, Identifier.Still));
		things.add(new Cloud(150,350, Identifier.Still));
		things.add(new Cloud(500,750, Identifier.Still));
		...
		p = new Portal(650,250, Identifier.Portal);
	...
	public void updateState() {
		...
		if (c.items() == 8) {
			things.add(p);
			if (spawn % 300 == 0) {
				things.add(new Boss(650,250, Identifier.Boss, this, things));
			}
			
		}
		
		if (c.hasWon()) {
			MenuOptions.setOption(game.nextState);
			music.stopMusic();
			music.setSong("#3.wav");
			music.startMusic();
		}
